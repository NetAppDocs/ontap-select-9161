...
.Hardware RAID
--
If you use a local hardware RAID controller, you must create at least one datastore (ESX) or one link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool[storage pool (KVM)] at each node for the system data as well as the root and data aggregates. You must attach the storage pool as part of configuring the ONTAP Select node.
--

.Software RAID
--
If you use software RAID, you must create at least one datastore (ESX) or one link:kvm-host-configuration-and-preparation-checklist.html#create-storage-pool[storage pool (KVM)] for the system data and make sure the SSD drives are available for the root and data aggregates. You must attach the storage pool and disks as part of configuring the ONTAP Select node.
--
====
